...
def enable_mlflow_local(
    ...
    try:
        ...
        log.info(f"MLflow experiment set: {experiment_name} (ID: {experiment.experiment_id})")
    ...
    try:
        ...
        if tracking_uri is None:
            ...
            if start_mlflow_server(port=port, kill_existing=kill_existing_server):
                ...
                log.warning("Failed to auto-start MLflow server, falling back to file backend")
                if use_adalflow_dir:
                    ...
                    log.info(f"Using AdalFlow directory for MLflow artifacts: {adalflow_dir}")
                else:
...
def get_mlflow_server_command(host: str = "0.0.0.0", port: int = 8080) -> str:
    ...
    return f"mlflow server --backend-store-uri {backend_path} --host {host} --port {port}"


def start_mlflow_server(host: str = "0.0.0.0", port: int = 8080, wait: bool = True, kill_existing: bool = False) -> bool:
                    ...
                    if PSUTIL_AVAILABLE:
                        for proc in psutil.process_iter(['pid', 'name', 'cmdline']):
                            try:
                                ...
                                if 'mlflow' in ' '.join(cmdline) and str(port) in ' '.join(cmdline):
        ...
        if wait:
            ...
            for _ in range(20):
                ...
                try:
                    with urllib.request.urlopen(f"http://localhost:{port}", timeout=1) as response:
                        if response.status == 200:
                            log.info(f"✅ MLflow server started successfully at http://{host}:{port}")
                            return True
                except:
                    continue
