"""NEBIUS AI ModelClient integration."""
...
def get_probabilities(completion: ChatCompletion) -> List[List[TokenLogProb]]:
    ...
    for c in completion.choices:
        ...
        for nebius_token_logprob in content:
            token = nebius_token_logprob.token
            logprob = nebius_token_logprob.logprob
            log_probs_for_choice.append(TokenLogProb(token=token, logprob=logprob))
        log_probs.append(log_probs_for_choice)
    return log_probs


class NebiusClient(ModelClient):
    __doc__ = r"""A component wrapper for the Nebius API client.

    Support both embedding and chat completion API.

    Args:
        api_key (Optional[str], optional): NEBIUS API key. Defaults to None.
    ...
    References:
        - API: https://docs.nebius.com/studio/inference/api
        - Models: https://docs.nebius.com/studio/inference/models
    """
    ...
    def __init__(
        ...
        r"""It is recommended to set the NEBIUS_API_KEY environment variable instead of passing it as an argument.
        ...
        self._base_url = "https://api.studio.nebius.ai/v1/"
    ...
    def init_sync_client(self):
        api_key = self._api_key or os.getenv("NEBIUS_API_KEY")
        if not api_key:
            raise ValueError("Environment variable NEBIUS_API_KEY must be set")
