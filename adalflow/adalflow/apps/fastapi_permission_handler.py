...
class ApprovalQueue:
    ...
    def get_pending_requests(self) -> List[ApprovalRequest]:
        ...
        return [
            metadata for metadata in self.request_metadata.values()
            if metadata.status == "pending"
        ]
...
class FastAPIPermissionHandler(PermissionManager):
    ...
    def __init__(
        ...
        super().__init__(
            approval_callback=self._api_approval_callback,
    ...
    def _register_endpoints(self):
        ...
        @self.app.get(f"{self.api_prefix}/pending", response_model=List[ApprovalRequest])
        async def get_pending_approvals():
            """Get all pending approval requests."""
            return self.approval_queue.get_pending_requests()
            
        @self.app.get(f"{self.api_prefix}/{{request_id}}", response_model=ApprovalRequest)
        ...
        async def approve_request(
            ...
            log.info(f"Approval endpoint called with URL request_id={request_id}, body={response}")
            if request_id not in self.approval_queue.request_metadata:
                log.error(f"Request {request_id} not found in metadata. Available: {list(self.approval_queue.request_metadata.keys())}")
                raise HTTPException(status_code=404, detail="Request not found")
        ...
        async def get_approval_stats():
            ...
            stats = {
                ...
                "approved": len([m for m in metadata.values() if m.status == "approved"]),
                "rejected": len([m for m in metadata.values() if m.status == "rejected"]),
    ...
    async def _api_approval_callback(self, request: FunctionRequest) -> ApprovalOutcome:
        ...
        log.info(f"Created approval request {request_id} for tool '{request.tool_name}'")
