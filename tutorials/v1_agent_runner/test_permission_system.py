...
async def test_async_execution():
    ...
    # Test prompt
    prompt_kwargs = {
        "input_str": "Read config.json and update it with new settings"
    }
...
async def test_streaming_execution():
    ...
    # Test prompt
    prompt_kwargs = {
        "input_str": "Search for news and write a summary to output.txt"
    }
    ...
    async for event in streaming_result.stream_to_json("permission_test_events.json"):
        ...
        if isinstance(event, RunItemStreamEvent):
            if event.name == "agent.tool_permission_request":
                printc(f"\n📋 Permission Request: {event.item.data.tool_name}", color="yellow")
...
async def test_auto_approval():
    ...
    # Test prompt
    prompt_kwargs = {
        "input_str": "Read data.csv and write a summary"
    }
...
def test_sync_execution_wrapper():
    ...
    # Test prompt
    prompt_kwargs = {
        "input_str": "Tell me what tools you have available"
    }
