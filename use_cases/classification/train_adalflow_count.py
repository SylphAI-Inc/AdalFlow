...
class ObjectCountPredData(DataClass):
    thought: str = field(metadata={"desc": "Your thought process here"})
    answer: int = field(
...
class ObjectCountTask(Component):
    def __init__(self, model_client, model_kwargs):
        ...
        template_2 = r"""<START_OF_SYSTEM_PROMPT>{{system_prompt}}</END_OF_SYSTEM_PROMPT>{{input_str}}"""
...
if __name__ == "__main__":
    ...
    # single prompt without you: -> 0.82 <SYSTEM> system prompt.<SYS>0.78 <START_OF_SYSTEM_PROMPT> system prompt.<END_OF_SYSTEM_PROMPT> =>0.84
    # text/ user role -> 0.76
